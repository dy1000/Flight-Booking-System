...
if (isset($_GET['id'])){
    $user_id = $_GET['id'];
    $select = "SELECT * FROM booking where user_id = '$user_id'";
    $result = mysqli_query($conn, $select) or die("An error has occurred! " .mysqli_error($conn));
    $data = $result->fetch_assoc();
    if ($data!=null){
        $flight_id = $data['flight_id'];
        $select = "SELECT * FROM flights where id = '$flight_id'";
        $result = mysqli_query($conn, $select) or die("An error has occurred! " .mysqli_error($conn));
    ...
    if (isset($_POST['cancel'])){
        $user_id = $_POST['id'];
        $delete = "DELETE FROM booking WHERE user_id = '$user_id'";
        $result = mysqli_query($conn, $delete) or die("An error has occurred! " .mysqli_error($conn));
        header('location:yourflights.php?id='.$user_id);
    }
...
<div class="about-section-box">
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-6 col-md-6 text-center py-4">
                <div class="inner-column">
                    <h1 style="text-align: left">Flights <span></span></h1>
                    <ul style="font-size: 22px; text-align: left;">

                        <li>
                            <strong> Booking Reference:</strong> <?php echo $data['booking_reference'];?>
                        </li>
                        <li>
                         <strong> Passengers:</strong>    <?php echo $data['passengers'] != null ?$data['passengers']: 0;?>
                        </li>
                        <li>
                           <strong>Flight Type: </strong> <?php echo $data['flight_type'];?>
                        </li>
                        <li>
                           <strong>Departure: </strong>  <?php echo $data['city_departure'];?>
                        </li>
                        <li>
                           <strong>Arrival: </strong> <?php echo $data['city_arrival'];?>
                        </li>
                        <li>
                            <strong> Date of arrival: </strong>  <?php echo $data['date_arrival'];?>
                        </li>
                        <li>
                           <strong> Date of departure:</strong> <?php echo $data['date_departure'];?>
                        </li>
                        <li>
                            <form action="yourflights.php?id=<?php echo $data['user_id'];?>" method="post">
                                <input type="hidden" name="id" value="<?php echo $data['user_id'];?>">
                                <input type="submit" name="cancel" value="cancel" class="btn btn-lg btn-danger">
