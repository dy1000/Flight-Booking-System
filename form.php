...
if (isset($_POST['book'],$_GET['id'])){
    $errors = [];
    $flight_id = $_GET['id'];
    $user_id = $_SESSION['id'];
    $user_name = $_SESSION['name'];
    $booking_reference = md5($user_name, false);
    ...
    if (strlen($security_number)!=3){
        $errors['security_number'] = 'please enter a valid security number';
    }
    if (count($errors) ==0) {
        $passenger_name1 = $_POST['fname'];
        $passenger_name2 = $_POST['fname_2'];
        $passengers = [];
        if (!empty($passenger_name1)&& !empty($passenger_name2)){
            array_push($passengers,$passenger_name1,$passenger_name2);
            $count_passengers = count($passengers);
            try {
                $insert = "INSERT INTO booking 
            (booking_reference,passengers,user_id, flight_id,card_number) VALUES ('$booking_reference','$count_passengers','$user_id','$flight_id', '$card_number')";
                $result = mysqli_query($conn, $insert) or die("An error has occurred " . mysqli_error($conn));
                $message = " You have successfully booked your flight! Flight number is $flight_id";
            } catch (Exception $exception){
                $errors['booking'] = "You have already booked!";
            }
        } else if (!empty($passenger_name1)){
            $passengers[] = $passenger_name1;
            $count_passengers = count($passengers);
            try {
                $insert = "INSERT INTO booking 
            (booking_reference,passengers,user_id, flight_id,card_number) VALUES ('$booking_reference','$count_passengers','$user_id','$flight_id', '$card_number')";
        ...
        } else if (!empty($passenger_name2)) {
            $passengers[] = $passenger_name2;
            $count_passengers = count($passengers);
            try {
                $insert = "INSERT INTO booking 
            (booking_reference,passengers,user_id, flight_id,card_number) VALUES ('$booking_reference','$count_passengers','$user_id','$flight_id', '$card_number')";
            ...
            try {
                $insert = "INSERT INTO booking 
            (booking_reference,user_id, flight_id,card_number) VALUES ('$booking_reference','$user_id','$flight_id', '$card_number')";
                $result = mysqli_query($conn, $insert) or die("An error has occurred " . mysqli_error($conn));
                $message = " You have successfully booked your flight! Flight number is $flight_id";
            } catch (Exception $exception){
                $errors['booking'] = "You have already booked!";
    ...
    //echo "user_id: $user_id flight_id: $flight_id";
}
?>
<!DOCTYPE html>
<html lang="en"><!-- Basic -->
<head>
	...
	<link rel="stylesheet" href="reservationstyle.php">
	<!-- Pickadate CSS -->
	...
	<!-- Start Reservation -->
	   ...
	   <section class="banner">
		...
		<div class="card-content">
			...
			<form action="form.php?id=<?php echo isset($flight_id) ? $flight_id : ''?>" method="post">
				<div class="form-row">
					...
					<div class="form-row">
						<h3> Passenger 2</h3>
						<input type="text" name="title_2" placeholder="Title">
					</div>

					<div class="form-row">
						<input type="text" name="fname_2" placeholder="Full Name">

					</div>

					<div class="form-row">
						<input type="text" name="email_2" placeholder="Email Address">
					</div>

					<div class="form-row">
						<h3> Card details</h3>
						<input type="text" name="card_number" placeholder="16-digit card number" required>
					</div>
